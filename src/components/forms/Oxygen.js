...
import OxygenHourlyLineChart from "../Charts/oxygenline";
import OxygenHourlyBarChart from "../Charts/oxygenbar";
...
export default function Oxygen({ navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const newData = await fetchSensorData(selectedLocation, "oxygen");

        if (newData.length) {
          const currentAQI = newData[0].oxygen !== null ? newData[0].oxygen : 0;
  ...
  const getDynamicColor = (value) => {
    const range = OXYGEN_RANGES.find((r) => value >= r.min && value <= r.max);
  ...
  const renderItem = ({ item }) => {
    ...
    const dynamicColor = getDynamicColor(item.oxygen);
    const remark = OXYGEN_RANGES.find((r) => item.oxygen >= r.min && item.oxygen <= r.max)?.remark;
    ...
    return (
      ...
      <View style={styles.dataRow}>
        ...
        <Text style={[styles.dataText, { color: dynamicColor }]}>{item.oxygen}</Text>
        <Text style={[styles.dataText, { color: dynamicColor }]}>{item.oxygenRemarks}</Text>
      </View>
      </TouchableOpacity>
    );
  };
    ...
    >
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              <AQIChart value={aqiValue} label="Oxygen" pollutantType="Oxygen" unit="%" />
              ...
              {showLineChart ? <OxygenHourlyLineChart data={datas} /> : <OxygenHourlyBarChart data={datas} />}
              ...
              <View style={styles.valueRow}>
                ...
                <Text style={styles.valueRowText}>Oxygen</Text>
        ...
        <LocationModal
                ...
                />
        <Footer navigation={navigation} activeScreen="Oxygen" />
        {selectedRemark && (
