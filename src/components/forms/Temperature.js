...
import TemperatureHourlyLineChart from "../Charts/temperatureline";
import TemperatureHourlyBarChart from "../Charts/temperaturebar";
...
export default function Temperature({ navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const newData = await fetchSensorData(selectedLocation, "temperature");

        if (newData.length) {
          const currentAQI = newData[0].temperature !== null ? newData[0].temperature : 0;
  ...
  const getDynamicColor = (value) => {
    const range = TEMPERATURE_RANGES.find((r) => value >= r.min && value <= r.max);
  ...
  const renderItem = ({ item }) => {
    ...
    const dynamicColor = getDynamicColor(item.temperature);
    const remark = TEMPERATURE_RANGES.find((r) => item.temperature >= r.min && item.temperature <= r.max)?.remark;
    ...
    return (
      ...
      <View style={styles.dataRow}>
        ...
        <Text style={[styles.dataText, { color: dynamicColor }]}>{item.temperature}</Text>
        <Text style={[styles.dataText, { color: dynamicColor }]}>{item.temperatureRemarks}</Text>
      </View>
      </TouchableOpacity>
    );
  };
    ...
    >
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              <AQIChart value={aqiValue} label="Temperature" pollutantType="Temperature" unit="°C" />
              ...
              {showLineChart ? <TemperatureHourlyLineChart data={datas} /> : <TemperatureHourlyBarChart data={datas} />}
              ...
              <View style={styles.valueRow}>
                ...
                <Text style={styles.valueRowText}>Temperature</Text>
        ...
        />
                ...
                />
        <Footer navigation={navigation} activeScreen="Temperature" />
        {selectedRemark && (
