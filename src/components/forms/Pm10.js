...
import PM10HourlyLineChart from "../Charts/pm10line";
import PM10HourlyBarChart from "../Charts/pm10bar";

export default function PM10({ navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const newData = await fetchSensorData(selectedLocation, "pm10");

        if (newData.length) {
          const currentAQI = newData[0].pm10 !== null ? newData[0].pm10 : 0;
  ...
  const getDynamicColor = (value) => {
    const range = PM10_RANGES.find((r) => value >= r.min && value <= r.max);
  ...
  const renderItem = ({ item }) => {
    ...
    const dynamicColor = getDynamicColor(item.pm10);
    const remark = PM10_RANGES.find((r) => item.pm10 >= r.min && item.pm10 <= r.max)?.remark;
    ...
    return (
      <TouchableOpacity onPress={() => handleRowPress(remark)}>
        ...
        <View style={styles.dataRow}>
          ...
          <Text style={[styles.dataText, { color: dynamicColor }]}>{item.pm10}</Text>
          <Text style={[styles.dataText, { color: dynamicColor }]}>{item.pm10Remarks}</Text>
        </View>
      </TouchableOpacity>
    );
  };
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              <AQIChart value={aqiValue} label="PM 10" pollutantType="PM10" unit="µg/m³" />
              ...
              {showLineChart ? <PM10HourlyLineChart data={datas} /> : <PM10HourlyBarChart data={datas} />}
              ...
              <View style={styles.valueRow}>
                ...
                <Text style={styles.valueRowText}>PM10</Text>
        ...
        />
        <Footer navigation={navigation} activeScreen="Pm10" />
        {selectedRemark && (
